...
function SearchPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const [whitelistRes, blacklistRes, cameraRes, locationRes] = await Promise.all([
          ...
          <CardContent>
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={3} display="flex" alignItems="center" gap={2}>
                ...
                <Button variant="outlined" color="secondary" onClick={() => setFilters({ plate_number: '', owner_name: '', owner_phone: '', contact_email: '', location_id: '', status: '', valid_status: '', approval_status: '', violation_type: '', severity: '', date_from: '', date_to: '', camera_id: '', q: '' })} sx={{ borderRadius: 2, px: 3, fontWeight: 600 }} disabled={loading}>
                  Làm mới
                </Button>
              </Grid>
            </Grid>
              ...
              <TableContainer>
                <Table>
                  ...
                  <TableBody>
                    {results.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={tab === 'camera' || tab === 'location' ? 5 : 8} align="center">
                          <Typography color="text.secondary" sx={{ py: 4 }}>Không có kết quả</Typography>
                        </TableCell>
                      </TableRow>
                    ) : (
                      results.map(item => (
                        <TableRow key={item.id || item.code || item.camera_key} hover>
                          {/* Row động theo tab */}
                          {tab === 'camera' ? (
                            <>
