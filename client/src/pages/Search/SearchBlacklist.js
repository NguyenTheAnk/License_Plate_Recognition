...
function SearchBlacklist({ 
  ...
  // Hàm gọi API lấy blacklist
  const handleSearch = async () => {
    setLocalLoading(true);
    setLocalError(null);
    try {
      const token = localStorage.getItem('token');
      const params = { ...filters, page: page + 1, limit: rowsPerPage };
      const res = await fetchDataFromAPI('/api/blacklist', token, { params });
      if (res.success) {
        setData(res.data || []);
        setSnackbar({
          open: true,
          message: `Tìm thấy ${res.data?.length || 0} kết quả`,
          severity: 'success'
        });
      } else {
        setData([]);
        ...
        <CardContent>
          <Grid container spacing={2}>
            ...
            <Grid item xs={12} md={2}>
              <FormControl fullWidth size="small">
                ...
                <Select 
                  ...
                  label="Loại vi phạm" 
                  onChange={e => onFilterChange('violation_type', e.target.value)}
                >
            ...
            <Grid item xs={12} md={3}>
              ...
              >
                {(localLoading || loading) ? <CircularProgress size={20} color="inherit" /> : 'Tìm kiếm'}
              </Button>
            </Grid>
          </Grid>
        </CardContent>
      ...
      <Card sx={{ borderRadius: 3, boxShadow: 1 }}>
        <CardContent sx={{ p: 0 }}>
          {(localLoading || loading) ? (
            ...
            <Box display="flex" justifyContent="center" alignItems="center" minHeight={200}>
              <Alert severity="error" sx={{ width: '100%', mx: 2 }}>
                {localError || error}
              </Alert>
            </Box>
          ) : (
            <>
              <TableContainer>
                <Table>
                  <TableHead>
                    <TableRow sx={{ background: 'linear-gradient(90deg, #d32f2f 0%, #f44336 100%)' }}>
                  ...
                  <TableBody>
                    {(data.length === 0 && results.length === 0) ? (
                      ...
                      (data.length > 0 ? data : results).map((item, idx) => (
                        <TableRow 
                          key={item.id} 
                          hover
                          sx={{ 
          ...
          {selectedItem && (
            <Grid container spacing={2}>
              <Grid item xs={12} md={4}>
                ...
                >
                  <DirectionsCar sx={{ fontSize: 40, color: '#f44336' }} />
                </Avatar>
                <Typography variant="h5" fontWeight={700} sx={{ mb: 1, color: 'error.main' }}>
              ...
              <Grid item xs={12} md={8}>
                ...
                <Box display="flex" alignItems="center" gap={2} mb={1}>
                  ...
                  <Box>
                    <Typography variant="caption" color="text.secondary">Loại vi phạm</Typography>
                    <Typography variant="body2" fontWeight={500}>
                  ...
                  <Box>
                    ...
                    <Typography variant="body2" fontWeight={500}>
                      {selectedItem.severity ? 
                        severityOptions.find(s => s.value === selectedItem.severity)?.label || selectedItem.severity
                        : 'Không xác định'
                      }
                    </Typography>
                  </Box>
