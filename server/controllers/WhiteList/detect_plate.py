...
device = select_device('cpu')  # Sử dụng CPU để tránh lỗi CUDA
print(f"Using device: {device}")

def load_yolo_model(weights_path):
    """Tải mô hình YOLOv5 từ file weights"""
    try:
    ...
    try:
        ...
        for det in pred:
            if len(det):
                ...
                plate_crop = img0[int(y1):int(y2), int(x1):int(x2)]
...
def recognize_plate_paddleocr(plate_img):
    """Nhận diện ký tự biển số sử dụng PaddleOCR"""
    try:
