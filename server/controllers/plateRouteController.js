...
// Tìm kiếm biển số tương tự để gợi ý
const searchSimilarPlates = async (req, res) => {
    try {
        const { query, limit = 10 } = req.query;
        
        console.log('🔍 Search similar plates request:', { query, limit });
        ...
        // Tìm kiếm biển số tương tự - sử dụng string concatenation để tránh lỗi prepared statement
        const searchPattern = `%${searchTerm}%`;
        const limitValue = parseInt(limit);
        
        console.log('🔍 Using string concatenation to avoid prepared statement issues');
        console.log('🔍 Search pattern:', searchPattern);
        console.log('🔍 Limit:', limitValue);
        
        // Sử dụng string concatenation thay vì prepared statement
        const searchQuery = `
            SELECT DISTINCT 
                plate_number,
            ...
            WHERE plate_number LIKE '${searchPattern}'
        ...
        const [rows] = await db.promise().execute(searchQuery);

        console.log('✅ Found suggestions:', rows.length, 'rows');
        console.log('📋 Suggestions data:', rows);
        ...
        console.error('Error searching similar plates:', error);
        return res.status(500).json({
            success: false,
            message: 'Lỗi server khi tìm kiếm biển số tương tự'
        });
    }
};
...
const searchPlateRoute = async (req, res) => {
    ...
    `;
        ...
        const orderedCameras = routeData.map(group => {
            // Xử lý timezone để đảm bảo ngày tháng chính xác
            const firstDetection = group.detections[0].detected_at;
            const localDate = new Date(firstDetection);
            ...
            return {
                ...
                first_detected_at: localDate.toISOString(), // Đảm bảo format ISO string
