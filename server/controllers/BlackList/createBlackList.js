...
const createBlacklist = async (req, res) => {
    ...
    try {
        ...
        const insertFields = [
            ...
            'owner_name', 'owner_phone', 'valid_from', 'valid_to', 'description', 'evidence_files',
        ...
        const insertValues = [
            location_id, plate_number || ocrText, vehicle_id || null, violation_type, reason, severity,
            owner_name || null, owner_phone || null, valid_from || null, valid_to || null, description || null,
    ...
    try {
        ...
        try {
            for (let i = 0; i < entries.length; i++) {
                ...
                const { location_id, plate_number, vehicle_id, violation_type, reason, severity, owner_name, owner_phone, valid_from, valid_to, description } = entry;

                try {
                    // Basic validation: location_id và reason là bắt buộc
                    if (!location_id || !reason) {
                        results.errors.push({
                            index: i,
                            entry,
                            error: 'location_id và reason là bắt buộc'
                        });
                        continue;
                    }
