...
const getCamerasForRouteMonitoring = async (req, res) => {
    const connection = await db.promise();
    ...
    try {
        const [cameras] = await connection.execute(`
            ...
            WHERE c.is_active = 1 
            AND l.is_active = 1
            AND l.Ox IS NOT NULL 
            AND l.Oy IS NOT NULL
            ORDER BY l.Ox, l.Oy
        `);
        ...
        res.status(200).json({
            ...
            data: {
                ...
                online_count: activeCameras.filter(c => c.connection_status === 'online').length,
                offline_count: activeCameras.filter(c => c.connection_status === 'offline').length,
                warning_count: activeCameras.filter(c => c.connection_status === 'warning').length
            }
        });

    } catch (error) {
        console.error('Error getting cameras for route monitoring:', error);
        res.status(500).json({
            success: false,
            message: 'Lỗi khi lấy danh sách camera cho giám sát lộ trình',
            error: process.env.NODE_ENV === 'development' ? error.message : undefined
        });
    }
};
