    ...
    try:
        ...
        if not os.path.isabs(model_path):
            ...
            if os.path.exists(full_path):
                ...
                # If it's yolov8n.pt, allow download
                if os.path.basename(model_path) == 'yolov8n.pt':
                    logger.info("📥 yolov8n.pt not found locally, will try download")
                    model_path = 'yolov8n.pt'  # Let YOLO download
                else:
                    logger.error(f"❌ File not found: {full_path}")
                    return None
        
        # FIXED: Enable download for yolov8n.pt
        ...
        if 'yolov8n.pt' in model_path:
            ...
            logger.info("Enabled download for yolov8n.pt")
        ...
        try:
            ...
            # ULTIMATE FIX: Use context manager with comprehensive globals (+Concat)
            try:
            ...
            if hasattr(torch.serialization, 'safe_globals'):
                # PyTorch 2.6+ with safe_globals context manager
                with torch.serialization.safe_globals(safe_globals_list):
                    logger.info("Using safe_globals context manager with comprehensive module list")
                ...
                # PyTorch 2.6+ with add_safe_globals
                torch.serialization.add_safe_globals(safe_globals_list)
                logger.info("Using add_safe_globals with comprehensive module list")
                model = YOLO(model_path)
                logger.info("✅ YOLO loading successful with add_safe_globals")
            else:
                # PyTorch < 2.6 - use direct loading
                logger.info("Using direct YOLO loading for older PyTorch version")
                model = YOLO(model_path)
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    # Also allow DFL block explicitly
                    try:
                        from ultralytics.nn.modules import block as _ultra_block
                        if hasattr(_ultra_block, 'DFL'):
                            allow.append(_ultra_block.DFL)
                    except Exception:
                        pass
                    if allow:
...
def check_model_availability():
    ...
    try:
        ...
        model_files = [
            'detect_license_plate_yolov8s.pt',
            'LP_detection_yolov11.pt',
            'yolov8n-seg.pt'
        ]
...
def test_ocr_extraction():
    """Test OCR extraction with a known good synthetic image"""
    try:
        global ocr_reader
        ...
        # Create a perfect test image
        test_img = np.ones((120, 400, 3), dtype=np.uint8) * 255  # White background
        cv2.rectangle(test_img, (50, 30), (350, 90), (0, 0, 0), -1)  # Black rectangle
        cv2.putText(test_img, "30A12345", (60, 70), cv2.FONT_HERSHEY_SIMPLEX, 1.5, (255, 255, 255), 3)
        
        logger.info("Created test image with '30A12345'")
        
        # Test OCR
        result = ocr_reader.ocr(test_img, det=True, rec=True, cls=False)
        logger.info(f"Test OCR result type: {type(result)}")
        
        # Debug the result structure
        debug_ocr_result_structure(result)
        
        # Try extraction
        text, conf = safe_extract_ocr_text(result)
        if text:
            logger.info(f"TEST SUCCESS: Extracted '{text}' with confidence {conf:.3f}")
            return True
        else:
            logger.error("TEST FAILED: No text extracted from perfect test image")
            return False
...
# Plate model candidates (ordered by preference)
...
# HARD-CODED selection for plate model.
# To test another model, change the string below to one of the files above
# that exists in the plate_recognition/ folder, then restart the server.
PLATE_MODEL_FIXED_NAME = 'detect_license_plate_yolov8s.pt'  # Use specialized license plate model
...
# DeepSORT tracker - OPTIMIZED FOR VEHICLE TRACKING with exact fit (robust CPU/GPU init)
tracker = None
def initialize_tracker():
    """Initialize tracker with proper error handling for DeepSORT requirements"""
    global tracker
    
    try:
        logger.info("Initializing tracking system...")
        
        # Strategy 1: Try DeepSORT with proper parameters
        try:
            from deep_sort_realtime.deepsort_tracker import DeepSort
            logger.info("DeepSort imported successfully")
            
            # FIXED: Initialize DeepSORT with proper parameters to avoid "embeddings or frame" error
            tracker = DeepSort(
                max_age=30,
                n_init=2,
                nms_max_overlap=0.3,
                ...
                bgr=True,
                embedder_gpu=False,
                # CRITICAL FIX: Add these parameters to prevent initialization error
                embedder_wts=None,  # Let DeepSORT use default weights
                embedder_model_name=None,  # Use default model
                today=None  # Use default date
            )
            
            # CRITICAL FIX: Test tracker with proper format INCLUDING FRAME
            logger.info("Testing DeepSORT with dummy data...")
            dummy_frame = np.ones((100, 100, 3), dtype=np.uint8) * 128
            
            # Correct DeepSORT detection format: [[bbox], confidence, class_id]
            test_detections = [
                [[50.0, 50.0, 40.0, 30.0], 0.8, 2]  # [x, y, w, h], conf, class
            ]
            
            # FIXED: Always provide frame parameter
            test_result = tracker.update_tracks(test_detections, frame=dummy_frame)
            
            if isinstance(test_result, list):
                logger.info(f"DeepSORT test successful: {len(test_result)} tracks returned")
                return True
            else:
                logger.error("DeepSORT test failed: invalid return type")
                raise Exception("DeepSORT test failed")
            
        except ImportError as import_error:
            logger.error(f"DeepSort import failed: {import_error}")
            logger.error("Please install: pip install deep-sort-realtime")
            
        except Exception as deepsort_error:
            logger.error(f"DeepSORT initialization failed: {deepsort_error}")
            logger.info("Falling back to simple tracker...")
        
        # Strategy 2: Enhanced Simple Tracker
        logger.info("Creating enhanced simple tracker...")
        ...
        class EnhancedSimpleTracker:
            def __init__(self):
                ...
                self.max_distance = 150  # pixels
                
            def update_tracks(self, detections, frame=None):
                """Enhanced simple tracking with better association"""
                try:
                    current_time = time.time()
                    current_tracks = []
                    
                    logger.debug(f"SimpleTracker: Processing {len(detections)} detections")
                    
                    # Convert detections to usable format
                    detection_boxes = []
                    for i, det in enumerate(detections):
                        try:
                            if len(det) >= 6:  # [x, y, w, h, conf, class]
                            ...
                            logger.debug(f"Error parsing detection {i}: {det_error}")
                            continue
                    ...
                    for track_id, track_data in list(self.active_tracks.items()):
                        ...
                        for det in detection_boxes:
                            ...
                            # Calculate distance
                            dx = det['center'][0] - track_data['center'][0]
                            dy = det['center'][1] - track_data['center'][1]
                        ...
                        if best_detection:
                            ...
                            used_detections.add(best_detection['id'])
                            self.active_tracks[track_id].update({
                                'bbox': best_detection['bbox'],
                                'center': best_detection['center'],
                                'confidence': best_detection['confidence'],
                                'class_id': best_detection.get('class_id', self.active_tracks[track_id].get('class_id', -1)),
                                'last_seen': current_time
                            })
                            
                            # Create track object
                            track_obj = self._create_track_object(track_id, best_detection['bbox'], self.active_tracks[track_id].get('class_id', -1))
                    ...
                    for det in detection_boxes:
                        if det['id'] not in used_detections:
                            ...
                            track_obj = self._create_track_object(track_id, det['bbox'], det['class_id'])
                            current_tracks.append(track_obj)
                    
                    logger.debug(f"SimpleTracker: {len(current_tracks)} active tracks")
            ...
            def _create_track_object(self, track_id, bbox, class_id):
                """Create track object compatible with existing code"""
                class SimpleTrack:
                    ...
                    def to_ltrb(self):
                        return np.array(self._bbox)  # [x1, y1, x2, y2]
                
                return SimpleTrack(track_id, bbox, class_id)
        
        tracker = EnhancedSimpleTracker()
        logger.info("Enhanced simple tracker created successfully")
        return True
        
    except Exception as e:
        logger.error(f"All tracker initialization failed: {e}")
        
        # Final fallback: Minimal tracker
        class MinimalTracker:
            def __init__(self):
                self.counter = 0
                
            def update_tracks(self, detections, frame=None):
                """Minimal tracker: just assign IDs to detections"""
                tracks = []
                for i, det in enumerate(detections):
                    if len(det) >= 6:
                        x, y, w, h = det[:4]
                        ...
                        self.counter += 1
                        track = MinimalTrack(self.counter, [x, y, x + w, y + h], class_id)
                        tracks.append(track)
                
                logger.debug(f"MinimalTracker: created {len(tracks)} tracks")
                return tracks
        
        tracker = MinimalTracker()
        logger.info("Minimal tracker created as final fallback")
        return True

# FIXED: Update the tracker test at the end of initialization
# Call initialization
try:
    init_success = initialize_tracker()
    if init_success and tracker:
        logger.info(f"Tracker ready: {type(tracker).__name__}")
    else:
        logger.error("Tracker initialization completely failed")
except Exception as e:
    logger.error(f"Tracker setup error: {e}")
except Exception as critical_error:
    logger.error(f"❌ Critical tracker initialization error: {critical_error}")
    class NoOpTracker:
        def update_tracks(self, detections, frame=None):
            logger.error("NoOp tracker: all tracking disabled")
            return []
    tracker = NoOpTracker()

logger.info(f"Final tracker type: {type(tracker)}")

# FIXED: Verify tracker works with proper frame parameter
if tracker:
    try:
        # Create a proper test frame (not None)
        test_frame = np.ones((100, 100, 3), dtype=np.uint8) * 128
        test_result = tracker.update_tracks([], frame=test_frame)  # FIXED: Provide frame
        logger.info(f"✅ Tracker test successful: {type(test_result)}")
    except Exception as test_error:
        logger.error(f"❌ Tracker test failed: {test_error}")
else:
    logger.error("❌ No tracker available!")
...
def safe_extract_ocr_text(ocr_result):
    ...
    try:
        ...
        return None, 0.0
def test_ocr_with_known_images():
    """Test OCR với những hình ảnh biết trước kết quả"""
    try:
        global ocr_reader
        ...
        logger.info("=== TESTING OCR WITH KNOWN IMAGES ===")
        
        # Test 1: Simple synthetic image (black text on white background)
        test1 = np.ones((80, 250, 3), dtype=np.uint8) * 255  # White background
        cv2.rectangle(test1, (10, 10), (240, 70), (255, 255, 255), -1)  # Black rectangle
        cv2.putText(test1, "30A-123", (20, 55), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 0, 0), 2, lineType=cv2.LINE_AA)
        
        result1 = debug_ocr_step_by_step(ocr_reader, test1, save_debug=True)
        if result1[0]:
            logger.info("TEST 1 PASSED: Simple synthetic")
        else:
            logger.error("TEST 1 FAILED: Simple synthetic")
            return False
        
        # Test 2: More realistic license plate (dark text)
        test2 = np.ones((60, 200, 3), dtype=np.uint8) * 255
        cv2.rectangle(test2, (5, 5), (195, 55), (230, 230, 230), -1)  # Light gray background
        cv2.putText(test2, "51B-789", (15, 45), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 0, 0), 2, lineType=cv2.LINE_AA)
        
        result2 = debug_ocr_step_by_step(ocr_reader, test2, save_debug=True)
        if result2[0]:
            logger.info("TEST 2 PASSED: Realistic plate")
        else:
            logger.error("TEST 2 FAILED: Realistic plate")
...
def test_ocr_engine():
    ...
    try:
        ...
        # Draw simple text-like pattern
        import cv2
        cv2.rectangle(test_img, (50, 30), (250, 70), (0, 0, 0), -1)  # Black rectangle
        cv2.putText(test_img, "30A123", (60, 55), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        
        logger.info("🧪 Created test image with '30A123'")
        
        # Test OCR
        if ocr_reader is not None:
            logger.info("🧪 Testing OCR reader...")
            result = ocr_with_auto_fallback(ocr_reader, test_img, det=True, rec=True, cls=False)  # FIXED: cls=False
            logger.info(f"🧪 OCR result: {result}")
            
            text, conf = safe_extract_ocr_text(result)
            logger.info(f"🧪 Extracted: '{text}' (conf: {conf})")
            
            return text is not None
        else:
            logger.error("🧪 OCR reader is None!")
            return False
...
def detect_license_plate_in_vehicle(vehicle_crop, plate_model):
    """Simplified plate detection using yolov8n.pt"""
    try:
...
def initialize_models_properly():
    """Simplified initialization using only yolov8n.pt"""
    global yolo_model, plate_model, plate_model_name, ocr_reader
    
    try:
        current_dir = os.path.dirname(os.path.abspath(__file__))
        
        # Load yolov8n.pt for both vehicle and plate detection
        ...
        plate_model = yolo_model
        plate_model_name = "yolov8n.pt"
        logger.info("✅ Using yolov8n.pt for both vehicle and plate detection")
        ...
        return False
def safe_deepsort_update(tracker, detections, frame):
...
def detect_and_ocr(frame, video_processing=True):
    global yolo_model, plate_model, ocr_reader, tracked_objects, frame_count, processing_deadline_ts
    
    # Simplified initialization - no debug mode overhead
    detection_attempted = False
    # CRITICAL: Always return a valid result to prevent video freezing
    try:
        if not hasattr(setup_pytorch_environment_fixed, 'completed'):
            setup_pytorch_environment_fixed()
            setup_pytorch_environment_fixed.completed = True
        
        # Quick model check - only initialize if needed
        if yolo_model is None or plate_model is None:
            initialize_models_properly()
        
        # Quick OCR check - only initialize if needed
        if ocr_reader is None:
            try:
                from paddleocr import PaddleOCR
        ...
        global last_frame_time, smoothed_fps
        now_ts = time.time()
        ...
        # STEP 6: Initialize response arrays
        boxes = []
        labels = []
        ocr_results = []
        
        # Initialize tracked_objects to ensure it's always available
        if not hasattr(detect_and_ocr, 'tracked_objects_initialized'):
            tracked_objects.clear()
            detect_and_ocr.tracked_objects_initialized = True
        
        # No fake detections - only real vehicle tracking
        
        # Always run detection for better results
        detections = []
        tracks = []
        ...
        try:
            ...
            logger.info(f"🔍 Detection result: {len(detections)} detections")
            
            # Quick tracking if we have detections
            if detections and tracker:
                tracks = safe_deepsort_update(tracker, detections, frame)
                logger.info(f"🎯 Tracking result: {len(tracks)} tracks")
        ...
        # Simplified raw detection handling - only draw boxes, no OCR
        if (not tracks or len(tracks) == 0) and detections:
            for i, det in enumerate(detections):
                if len(det) >= 6:
                    x, y, w, h, conf, class_id = det[:6]
                    ...
                    cv2.putText(display_frame, f"VEH C{int(class_id)} {float(conf):.2f}", (x1, max(10, y1-8)),
                               cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                    boxes.append([x1, y1, x2, y2])
        
        # STEP 8: Run per-track processing ONCE and reuse (avoid duplicate OCR per frame)

        # Optimized track processing - only OCR every 5 frames
        ui_tracks = []
        track_results_cache = {}

        # Process tracks with OCR only every 5 frames for better performance
        for track in tracks:
            try:
                if hasattr(track, 'track_id') and hasattr(track, 'to_ltrb'):
                    ...
                    if tr and isinstance(tr, dict):
                        track_results_cache[int(track.track_id)] = tr
            except Exception as e:
                pass  # Skip error logging for performance

        # Simplified track drawing for better performance
        for track in tracks:
            try:
                ...
                # Get cached result
                track_data = track_results_cache.get(int(track_id))
                plate_text = track_data.get('plate_number') if track_data else None
                confidence = track_data.get('confidence', 0.0) if track_data else 0.0
                
                # Simple color scheme
                color = (0, 255, 0) if (plate_text and plate_text != 'Đang nhận diện...') else (255, 255, 0)
                ...
                # Simple label
                class_id = getattr(track, 'class_id', -1)
                vehicle_name = 'Car' if class_id == 2 else 'Vehicle'
                ocr_display = plate_text if (plate_text and plate_text != 'Đang nhận diện...') else '...' 
                top_label = f"{vehicle_name}: T{track_id} | {ocr_display}"
                
                # Draw label background
                cv2.rectangle(display_frame, (x1, max(10, y1-25)), (x1 + len(top_label) * 8, y1 - 5), (0, 0, 0), -1)
                cv2.putText(display_frame, top_label, (x1 + 5, y1 - 8), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
                
                # Simplified plate drawing - only if we have plate data
                if track_data and track_data.get('plate_frame_bbox'):
                    try:
                        px1, py1, px2, py2 = map(int, track_data['plate_frame_bbox'][:4])
                        if (px1 < px2 and py1 < py2 and px1 >= 0 and py1 >= 0 and 
                            px2 <= original_width and py2 <= original_height):
                            
                            # Draw plate bounding box
                            cv2.rectangle(display_frame, (px1, py1), (px2, py2), (255, 255, 255), 2)
                            
                            # Draw OCR result if available
                            if plate_text and plate_text != 'Đang nhận diện...':
                                plate_label = f"{plate_text} ({confidence:.2f})"
                                cv2.rectangle(display_frame, (px1, max(10, py1-20)), (px1 + len(plate_label) * 8, py1 - 5), (0, 0, 0), -1)
                                cv2.putText(display_frame, plate_label, (px1 + 5, py1 - 8), 
                                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
                                
                                labels.append(plate_text)
                                ocr_results.append([plate_text, confidence])
                    except:
                        pass
                
                boxes.append([x1, y1, x2, y2])

                # If we have a previously consistent result, persist it even if OCR fails this frame
                if track_id in track_consistency:
                    cons = track_consistency[track_id]
                    if cons.get('best_result'):
                        if plate_text is None or plate_text == 'Đang nhận diện...':
                            plate_text = cons['best_result']
                            confidence = cons.get('best_confidence', confidence)
                            # update label on frame with enhanced visibility
                            vehicle_name = _class_name(getattr(track, 'class_id', -1))
                            top_label = f"{vehicle_name}: T{track_id} | {plate_text}"
                            label_w = max(140, len(top_label) * 10)
                            cv2.rectangle(display_frame, (x1, max(10, y1-28)), (x1 + label_w, y1 - 6), (0, 0, 0), -1)
                            cv2.rectangle(display_frame, (x1, max(10, y1-28)), (x1 + label_w, y1 - 6), (0, 255, 0), 2)
                            cv2.putText(display_frame, top_label, (x1 + 6, y1 - 10), 
                                       cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 2)
                
            except Exception as e:
                logger.debug(f"Error processing track {i}: {e}")

        # Simplified status - only essential info
        try:
            # FPS only
            fps_text = f"FPS: {smoothed_fps:.1f}"
            cv2.putText(display_frame, fps_text, (original_width - 100, 25), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
            
            # Simple legend
            legend_y = original_height - 40
            cv2.putText(display_frame, "GREEN: Tracked + OCR", (10, legend_y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            cv2.putText(display_frame, "YELLOW: Detecting", (10, legend_y + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        except:
            pass

        # STEP 9: Success message removed for cleaner display

        # Simplified tracked objects update
        for track in tracks:
            try:
                if hasattr(track, 'track_id'):
                    ...
                    if track_data:
                        plate_text = track_data.get('plate_number')
                        ...
                        if track_id not in tracked_objects:
                            tracked_objects[track_id] = {
                                'track_id': track_id,
                                'bbox': bbox,
                                'plate_number': plate_text or 'Đang nhận diện...',
                                'confidence': confidence,
                                'first_seen': time.time(),
                                'last_seen': time.time(),
                                'disappeared': 0,
                                'validation_passed': bool(plate_text and confidence > 0.05),
                                'saved_to_db': False
                            }
                        else:
                            # Update existing
        ...
        result = {
            ...
            'ids': [t.track_id for t in tracks if hasattr(t, 'track_id')],
            'tracks': ui_tracks,
            'frame_width': int(original_width),
            'frame_height': int(original_height),
            'detection_this_frame': bool(detection_attempted)
        }
        
        return result
        
    except Exception as critical_error:
        logger.error(f"CRITICAL ERROR in detect_and_ocr: {critical_error}")
        
        # MUST return valid response to prevent video freeze
        error_frame = np.zeros((480, 640, 3), dtype=np.uint8)
        cv2.putText(error_frame, "PROCESSING ERROR", (50, 200), 
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
        cv2.putText(error_frame, "Video will continue...", (50, 250), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        
        frame_bytes = safe_frame_encoding(error_frame)
        return {
            'frame': frame_bytes if frame_bytes else b'',
            'boxes': [], 'labels': [], 'ocr_results': [], 
            'tracked_objects': {}, 'ids': []
        }
...
if __name__ == "__main__":
    try:
        ...
        # Test multiple model paths
        test_paths = [

            os.path.join(current_dir, 'detect_license_plate_yolov8s.pt'),
            os.path.join(current_dir, 'LP_detection_yolov11.pt'),
            'yolov8n.pt'  # Try direct name
        ]
        ...
        if test_model:
            ...
            try:
                from ultralytics import YOLO
                test_model = YOLO('detect_license_plate_yolov8s.pt')
        ...
        try:
            if yolo_model is None:
                ...
                yolo_model = safe_yolo_load('detect_license_plate_yolov8s.pt')
            
            if plate_model is None:
                logger.info("Loading test plate model...")
                plate_model = safe_yolo_load('detect_license_plate_yolov8s.pt')
...
def test_visualization():
    """Test visualization with fake data"""
    try:
        test_frame = np.zeros((480, 640, 3), dtype=np.uint8)
        
        # Add some content
        cv2.putText(test_frame, "VISUALIZATION TEST", (200, 50), 
        ...
        if result and 'frame' in result:
            result_frame = cv2.imdecode(np.frombuffer(result['frame'], np.uint8), cv2.IMREAD_COLOR)
        else:
            result_frame = test_frame
        
        # Save test result
        cv2.imwrite('test_visualization.jpg', result_frame)
        logger.info("Test visualization saved as test_visualization.jpg")
