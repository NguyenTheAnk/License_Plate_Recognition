...
def detect_and_ocr_stable(frame, camera_id=None, source_type="camera", video_filename=None, camera_location=None, camera_name=None):
    ...
    # ENHANCED: STABLE TRACKING SYSTEM - Mỗi phương tiện chỉ có 1 track_id duy nhất
    current_track_ids = set()
    
    logger.info(f"🔍 ByteTracker results: {len(tracks)} tracks found")
    
    # Tạo dictionary để quản lý track ổn định
    stable_tracks = {}  # {track_id: {'plate': '', 'confidence': 0, 'bbox': [], 'frame_count': 0}}
    ...
    for track in tracks:
        ...
        # Process plate text
        processed_text = ""
        if best_plate_text and best_conf_val > MIN_CONFIDENCE and best_ocr_conf > MIN_OCR_CONFIDENCE:
            processed_text = process_plate_text(best_plate_text)
        
        # Tìm track tương tự đã tồn tại
        similar_track_id = None
        for existing_id, existing_data in stable_tracks.items():
            if 'bbox' in existing_data and existing_data['bbox']:
                # Tính khoảng cách giữa các bbox
                ex_x1, ex_y1, ex_x2, ex_y2 = existing_data['bbox']
                distance = ((x1 + x2)/2 - (ex_x1 + ex_x2)/2)**2 + ((y1 + y2)/2 - (ex_y1 + ex_y2)/2)**2
                distance = distance**0.5
                
                if distance < 50:  # Nếu gần nhau hơn 50 pixels
                    similar_track_id = existing_id
                    break
        
        # Nếu tìm thấy track tương tự, cập nhật track đó
        if similar_track_id:
            # Kiểm tra xem có phải cùng biển số không
            existing_plate = stable_tracks[similar_track_id]['plate']
            
            if processed_text and processed_text == existing_plate:
                # Cùng biển số - chỉ cập nhật bbox và frame count
                stable_tracks[similar_track_id]['frame_count'] += 1
                stable_tracks[similar_track_id]['bbox'] = [x1, y1, x2, y2]
                stable_tracks[similar_track_id]['last_seen'] = curr_time
                logger.debug(f"🔄 Same plate '{processed_text}' - updated bbox for track {similar_track_id}")
            elif processed_text and best_conf_val > stable_tracks[similar_track_id]['confidence']:
                # Biển số khác nhưng confidence cao hơn - cập nhật
                stable_tracks[similar_track_id] = {
                    'plate': processed_text,
                    'confidence': best_conf_val,
                    'bbox': [x1, y1, x2, y2],
                    'frame_count': stable_tracks[similar_track_id]['frame_count'] + 1,
                    'last_seen': curr_time
                }
                logger.info(f"🔄 Updated similar track {similar_track_id}: '{processed_text}' (conf: {best_conf_val:.3f}) - BETTER RESULT")
            else:
                # Cập nhật frame count và bbox nhưng giữ nguyên biển số
                stable_tracks[similar_track_id]['frame_count'] += 1
                stable_tracks[similar_track_id]['bbox'] = [x1, y1, x2, y2]
                stable_tracks[similar_track_id]['last_seen'] = curr_time
                logger.debug(f"🔄 Updated bbox for track {similar_track_id} - keeping existing plate '{existing_plate}'")
        else:
            # Tạo track mới - CHỈ TẠO KHI CÓ BIỂN SỐ HỢP LỆ
            if processed_text:
                # Kiểm tra xem biển số này đã tồn tại trong track khác chưa
                existing_plate_track = None
                for existing_id, existing_data in stable_tracks.items():
                    if existing_data['plate'] == processed_text:
                        existing_plate_track = existing_id
                        break
                
                if existing_plate_track:
                    # Biển số đã tồn tại - gộp vào track cũ
                    logger.info(f"🔄 Plate '{processed_text}' already exists in track {existing_plate_track} - merging with track {current_track_id}")
                    stable_tracks[existing_plate_track]['frame_count'] += 1
                    stable_tracks[existing_plate_track]['bbox'] = [x1, y1, x2, y2]
                    stable_tracks[existing_plate_track]['last_seen'] = curr_time
                    # Cập nhật confidence nếu tốt hơn
                    if best_conf_val > stable_tracks[existing_plate_track]['confidence']:
                        stable_tracks[existing_plate_track]['confidence'] = best_conf_val
                        logger.info(f"🔄 Updated confidence for existing plate '{processed_text}' to {best_conf_val:.3f}")
                else:
                    # Biển số mới - tạo track mới
                    ...
                    logger.info(f"🆕 New stable track {current_track_id}: '{processed_text}' (conf: {best_conf_val:.3f})")
            # KHÔNG TẠO TRACK NẾU KHÔNG CÓ BIỂN SỐ HỢP LỆ
    
    # Xử lý trường hợp không có tracks từ ByteTracker nhưng có detections
    if len(tracks) == 0 and len(detections) > 0:
        ...
        if best_detection and best_plate_text:
            ...
            if processed_text:
                ...
                # Thêm vào stable_tracks - kiểm tra duplicate trước
                existing_plate_track = None
                for existing_id, existing_data in stable_tracks.items():
                    if existing_data['plate'] == processed_text:
                        existing_plate_track = existing_id
                ...
                if existing_plate_track:
                    # Biển số đã tồn tại - gộp vào track cũ
                    logger.info(f"🔄 Direct detection plate '{processed_text}' already exists in track {existing_plate_track} - merging")
                    stable_tracks[existing_plate_track]['frame_count'] += 1
                    stable_tracks[existing_plate_track]['bbox'] = [plate_x1, plate_y1, plate_x2, plate_y2]
                    stable_tracks[existing_plate_track]['last_seen'] = curr_time
                    if best_conf_val > stable_tracks[existing_plate_track]['confidence']:
                        stable_tracks[existing_plate_track]['confidence'] = best_conf_val
                else:
    ...
    # Hiển thị stable tracks trên frame - HIỂN THỊ NGAY LẬP TỨC
    for track_id, track_data in stable_tracks.items():
        ...
        if bbox:
            ...
            # HIỂN THỊ NGAY LẬP TỨC - không chờ frame_count
            if plate_text and plate_text != "Đang nhận diện...":
                # Display on frame - HIỂN THỊ NGAY
                display_text = f"ID: {track_id} {plate_text}"
                if frame_count >= 5:  # Mark as stable
                    display_text += " ✓"
                ...
                # Add to response arrays
                boxes.append([x1, y1, x2, y2])
                labels.append(f"ID: {track_id} - {plate_text}")  # Format: ID: track_id + biển số
                ocr_results.append([plate_text, confidence])
                
                # Chỉ gửi database khi track đã ổn định (ít nhất 5 frame)
                if frame_count >= 5:
                    # Kiểm tra xem track này đã có biển số chưa
                    track_plate_key = f"track_plate_{track_id}"
                    
                    # Nếu track chưa có biển số hoặc biển số mới có confidence cao hơn
                    should_save = False
                    if track_plate_key not in plate_history:
                        should_save = True
                        logger.info(f"🎯 NEW TRACK {track_id}: '{plate_text}' (conf: {confidence:.3f}, frames: {frame_count}) - FIRST PLATE")
                    else:
                        existing_data = plate_history[track_plate_key]
                        existing_plate = existing_data.get('plate_text', '')
                        existing_conf = existing_data.get('confidence', 0.0)
                        existing_time = existing_data.get('timestamp', 0)
                        
                        # Chỉ cập nhật nếu biển số mới có confidence cao hơn đáng kể (ít nhất 20%)
                        if confidence > existing_conf * 1.2:
                            should_save = True
                            logger.info(f"🔄 UPDATE TRACK {track_id}: '{existing_plate}' -> '{plate_text}' (conf: {existing_conf:.3f} -> {confidence:.3f})")
                        else:
                            logger.debug(f"⏭️ TRACK {track_id}: Biển số '{plate_text}' (conf: {confidence:.3f}) không tốt hơn '{existing_plate}' (conf: {existing_conf:.3f})")
                    
                    if should_save:
                        logger.info(f"🎯 STABLE TRACK {track_id}: '{plate_text}' (conf: {confidence:.3f}, frames: {frame_count}) - SENDING TO DATABASE")
                        
                        # Save crop image
                        clean_plate_text = re.sub(r'[\\/*?:"<>|]', "_", plate_text)
                        crop_filename = f"plate_stable_{track_id}_{clean_plate_text}_{int(curr_time)}.jpg"
                        ...
                        try:
                            ...
                            if crop.size > 0:
                                ...
                                success_save = cv2.imwrite(crop_path, crop)
                                if success_save:
                                    logger.info(f"✅ Stable track {track_id} crop image saved: {crop_path}")
                                    ...
                                    if ENABLE_THREADING:
                                        ...
                                        logger.info(f"🚀 Stable track {track_id} plate '{plate_text}' queued for database")
                                    else:
                                        # Send sync
                                        ...
                                        logger.info(f"🚀 Stable track {track_id} plate '{plate_text}' sent to database: {success}")
                        except Exception as e:
                            logger.error(f"Error saving stable track {track_id} crop image: {e}")
                        
                        # Record as sent to avoid spam - lưu thông tin track và biển số
                        plate_history[track_plate_key] = {
                            'plate_text': plate_text,
                            'confidence': confidence,
                            'timestamp': time.time(),
                            'saved_once': True
                        }
                        logger.info(f"✅ Đã lưu biển số ổn định '{plate_text}' cho track {track_id} - chỉ lưu 1 lần duy nhất")
                    else:
                        logger.debug(f"⏭️ Track {track_id} đã có biển số tốt hơn, bỏ qua '{plate_text}'")
                
                current_track_ids.add(track_id)
                logger.info(f"✅ DISPLAY TRACK {track_id}: '{plate_text}' (conf: {confidence:.3f}, frames: {frame_count})")
            else:
                # Hiển thị track không có biển số - CHỈ HIỂN THỊ ID
                display_text = f"ID: {track_id}"
                
                text_y = max(y1 - 20, 20)
                cv2.putText(display_frame, display_text, (x1, text_y),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
                # Draw bounding box (YELLOW for tracks without plates)
                cv2.rectangle(display_frame, (x1, y1), (x2, y2), (0, 255, 255), 2)
                
                current_track_ids.add(track_id)
                logger.debug(f"🔄 TRACK WITHOUT PLATE {track_id}: (frames: {frame_count})")

    # Cleanup old stable tracks (older than 10 seconds)
    current_time = time.time()
    old_tracks = []
    for track_id, track_data in stable_tracks.items():
    ...
    for track_id in old_tracks:
        if track_id in stable_tracks:
            del stable_tracks[track_id]
            logger.debug(f"🧹 Cleaned up old stable track: {track_id}")
    
    # Cleanup old plate history (older than 60 seconds)
    if len(plate_history) > 100:
        old_plates = []
        for plate_key, plate_data in plate_history.items():
            # Xử lý cả dạng cũ (timestamp) và dạng mới (dict)
            if isinstance(plate_data, dict):
                save_time = plate_data.get('timestamp', 0)
            else:
                save_time = plate_data
            
            if current_time - save_time > 60.0:
                old_plates.append(plate_key)
        
        for plate_key in old_plates:
            del plate_history[plate_key]
        
        if old_plates:
            logger.debug(f"🧹 Cleaned up {len(old_plates)} old plate history records")
    
    # Less frequent cleanup to improve FPS stability
    if frame_count % 1000 == 0:  # Every 1000 frames
        cleanup_tracked_objects()

    # OPTIMIZED: Balanced JPEG quality for stable FPS
    result = {
        'frame': cv2.imencode('.jpg', display_frame, [cv2.IMWRITE_JPEG_QUALITY, 40])[1].tobytes(),
        'boxes': boxes,
        'labels': labels,
        'ocr_results': ocr_results,
        'tracked_objects': stable_tracks.copy(),  # Sử dụng stable_tracks thay vì tracked_objects
        ...
        'track_count': len(stable_tracks),  # Sử dụng stable_tracks
        'stable_tracks': len([t for t in stable_tracks.values() if t['frame_count'] >= 5]),  # Số track ổn định
        'skipped': should_skip
    }
    
    logger.debug(f"Frame {frame_count} completed: {len(boxes)} boxes, {len(labels)} labels, {len(stable_tracks)} stable tracks")
...
def get_detection_stats():
    ...
    try:
        ...
        # Get stable tracks from the last call (if available)
        # Note: This is a simplified version since stable_tracks is local to detect_and_ocr_stable
        stats = {
            ...
            'tracking_system': 'stable_tracking_v2'  # Indicate new tracking system
        }
